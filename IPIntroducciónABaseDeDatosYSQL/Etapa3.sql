...
/*3. Modifica el ejercicio anterior para mostrar en mayúsculas los valores cargados en el campo
...
SELECT *, DATEDIFF(CURDATE(), FECHAPEDIDO) AS 'DIAS TRANSCURRIDOS', DAYNAME(FECHAPEDIDO) AS DIA, DAYOFYEAR(FECHAPEDIDO) AS 'DIA DEL AÑO', MONTHNAME(FECHAPEDIDO) 
AS 'MES' FROM PEDIDOS_NEPTUNO;
...
#SELECT YEAR(FECHA_PEDIDO) AS 'AÑO', MONTHNAME(FECHA_PEDIDO) AS MES, COUNT(NUMERO_PEDIDO) AS VENTAS 
